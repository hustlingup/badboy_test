...
# Template for the new Soft Pop design for Advanced Version
...
<body>
    ...
    <div class="container mt-5 text-center">
        <div class="hero" style="padding: 1rem 0;">
            <p style="font-size: 1.5rem; color: #636e72; margin-bottom: 0.5rem;">테스트 결과</p>
        </div>
        ...
        <div class="result-card" style="text-align: center;">
            ...
            <div id="resultContainer" class="mb-4">
                <!-- Result gauges will be injected here by JavaScript -->
            </div>

            <div class="result-description">
                <h4 style="font-weight: 800; color: #2d3436; margin-bottom: 1rem;">{title}</h4>
                <div style="font-size: 1.1rem; color: #636e72; line-height: 1.8; text-align: left; padding: 0 1rem;">
                    {description}
                </div>
            </div>
        </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 1rem; max-width: 400px; margin: 0 auto;">
            <button class="btn btn-warning" onclick="window.location.href='all.html'">모든 유형 보기</button>
            <button class="btn btn-danger" onclick="window.location.href='index.html'">다시하기</button>
            <button class="btn btn-primary" onclick="window.location.href='../index.html'">일반 버전 하러가기</button>
        </div>

        <div class="result-card mt-5" style="text-align: center;">
            <h5><b>공유하기</b></h5>
            <div style="display: flex; justify-content: center; gap: 2rem; margin-top: 1.5rem;">
                <input type="text" value="https://badboytest.info/advance_version/{filename}" id="copy-link" hidden>
    ...
    <footer class="footer">
        <div class="container">
            <img src="../advanced_version/img/logo.png" alt="Logo" class="img-fluid mb-3" width="50">
            <h5 class="text-uppercase">ADVANCED BAD BOY TEST</h5>
            <p>나쁜남자 테스트 심화버전</p>
            <div class="mt-3">
...
def process_file(filepath):
    ...
    # Extract Title (e.g., <h4><b>순둥순둥~ 솜사탕 같은 착한남자</b></h4>)
    title_match = re.search(r'<h4><b>(.*?)</b></h4>', content)
    title = title_match.group(1).strip() if title_match else ""

    # Extract Description (Text after title)
    # Pattern: </h4>\s*<p>(.*?)</p>
    desc_match = re.search(r'</h4>\s*<p>(.*?)</p>', content, re.DOTALL)
...
# Process advanced version results
...
print("Done updating advanced results.")
