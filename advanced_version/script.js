$(document).ready(function() {
    ...
    const questions = [
        { question: "1. 그는 다른 사람들이 자신을 존경해 주기를 바라는 경향이 있다.", responses: ["전혀 그렇지 않다", "조금 그렇다", "보통이다", "대체로 그렇다", "매우 그렇다"] },
        { question: "2. 그는 사람들이 자신에게 관심을 가져주기를 바라는 경향이 있다", responses: ["전혀 그렇지 않다", "조금 그렇다", "보통이다", "대체로 그렇다", "매우 그렇다"] },
        { question: "3. 그는 다른 사람에게서 특별한 호의를 기대하는 경향이 있다.", responses: ["전혀 그렇지 않다", "조금 그렇다", "보통이다", "대체로 그렇다", "매우 그렇다"] },
        { question: "4. 그는 명성이나 지위를 추구하는 경향이 있다.", responses: ["전혀 그렇지 않다", "조금 그렇다", "보통이다", "대체로 그렇다", "매우 그렇다"] },
        { question: "5. 그는 자신의 뜻대로 하기 위해 속임수를 쓰거나 거짓말을 한다.", responses: ["전혀 그렇지 않다", "조금 그렇다", "보통이다", "대체로 그렇다", "매우 그렇다"] },
        { question: "6. 그는 자신의 뜻대로 되도록 다른 사람을 조종하는 경향이 있다.", responses: ["전혀 그렇지 않다", "조금 그렇다", "보통이다", "대체로 그렇다", "매우 그렇다"] },
        { question: "7. 그는 자신의 뜻대로 하기 위해 아첨을 한다.", responses: ["전혀 그렇지 않다", "조금 그렇다", "보통이다", "대체로 그렇다", "매우 그렇다"] },
        { question: "8. 그는 자신의 목적을 위해 다른 사람을 착취하는 경향이 있다.", responses: ["전혀 그렇지 않다", "조금 그렇다", "보통이다", "대체로 그렇다", "매우 그렇다"] },
        { question: "9. 그는 냉담하거나 무감각한 경향이 있다.", responses: ["전혀 그렇지 않다", "조금 그렇다", "보통이다", "대체로 그렇다", "매우 그렇다"] },
        { question: "10. 그는 냉소적인 편이며, 쌀쌀한 태도로 사람을 업신여기고 비웃곤 한다.", responses: ["전혀 그렇지 않다", "조금 그렇다", "보통이다", "대체로 그렇다", "매우 그렇다"] },
        { question: "11. 그는 잘못한 일에 대해 죄책감이 부족한 편이다.", responses: ["전혀 그렇지 않다", "조금 그렇다", "보통이다", "대체로 그렇다", "매우 그렇다"] },
        { question: "12. 그는 자신의 행동의 도덕성에 대해 별로 신경 쓰지 않는 편이다", responses: ["전혀 그렇지 않다", "조금 그렇다", "보통이다", "대체로 그렇다", "매우 그렇다"] }
    ];
    ...
    if (window.location.pathname.includes('quiz.html')) {
        ...
        questions.forEach((q, index) => {
            ...
            const responsesList = document.createElement('ul'); // Create <ul> for responses
            responsesList.style.listStyleType = 'none'; // Remove default bullets
            
            q.responses.forEach((response, i) => {
                const responseItem = document.createElement('li'); // Create <li> for each response
                const responseLabel = document.createElement('label');
                // Set text-align: left style to align text left (optional)
                responseLabel.style.textAlign = 'left';
                responseLabel.innerHTML = `<input type="radio" name="question${index}" value="${i}"> ${response}`;
                responseItem.appendChild(responseLabel);
                responsesList.appendChild(responseItem);
            });
            
            questionDiv.appendChild(responsesList); // Append <ul> to questionDiv
                ...
                <div>
                    ...
                    <div>
                        ...
                        <p>
                            ...
                            그럼에도 불구하고, 그는 매력적이고 사람을 끌어당기는 힘이 있으며, 이러한 매력을 바탕으로 대인관계를 이어갑니다. 때로는 기만적이고 성적으로 착취적인 모습도 보일 수 있지만, 그의 매력은 쉽게 사라지지 않습니다. 당신은 이러한 그의 복잡한 성격에도 불구하고, 여전히 그의 매력에 빠져들어 사랑하게 되었을 것입니다.                        </p>
                    </div>
