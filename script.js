$(document).ready(function () {
    ...
    // Questions data
    const questions = [
        { question: "이 남자의 외모는?", responses: ["보통이다", "가끔 잘생겼다는 이야기를 듣는다", "훈남이다", "너무 잘생겨서 불안하다"], image: '../img/qimage01.webp' },
        { question: "이 남자에게 무엇인가 해달라고 하면?", responses: ["무리한 부탁도 들어준다", "일부 들어준다", "이런 저런 이유로 안 된다고 하면서 빼려고 한다", "내가(여자가) 뭐라도 더 해주고싶다"], image: '../img/qimage02.webp' },
        { question: "이 남자가 고백 또는 사귀자는 이야기를", responses: ["남자가 먼저했다", "내가(여자가) 먼저 했다", "할 낌새가 안보여 애간장이 탄다", "안했고 앞으로도 안할것이 분명하지만 괜찮다"], image: '../img/qimage03.webp' },
        { question: "이 남자의 스킨쉽의 정도는?", responses: ["여자를 지켜주려 한다", "일반적이다", "서투르지만 많다", "아주 많고 능숙하다"], image: '../img/qimage04.webp' },
        { question: "이 남자가 헤어짐에 대해 어떻게 생각하는가?", responses: ["헤어지지 않으려 집착한다", "잘 모르겠다", "헤어지자고 한적이 있다", "미련이 없다/없어보인다"], image: '../img/qimage05.webp' },
        { question: "이 남자 주변에", responses: ["여자가 없고 일편단심이다", "여사친이 한두명 있다", "여사친이 많다", "모든 여자를 완전 거부하지도, 엄청잘해주지도 않는다"], image: '../img/qimage06.webp' },
        { question: "이 남자의 감정은", responses: ["드러나거나 공유된다", "숨기지만 알 수 있다", "잘 알다가도 모르겠다", "도저히 알수가 없다"], image: '../img/qimage07.webp' },
        { question: "이 남자가 무엇인가 결정할 때 내(여자의) 의사를", responses: ["항상 묻고 결정한다", "가끔 묻고 결정한다", "묻긴 하지만 자기 멋대로 한다", "묻지 않고 멋대로 행동해 좀 챙겨주고싶다."], image: '../img/qimage08.webp' }
    ];
    ...
    function showQuestion(index) {
        ...
        const questionHTML = `
            <div class="question-card">
                <img src="${question.image}" alt="Question Image" class="img-fluid mb-3" style="border-radius: 20px; width: 100%;">
                <h4 class="question-text">${question.question}</h4>
            </div>
            <div class="options-grid">
                ${shuffledResponsePointsMap.map((item, index) => `
                    <button class="option-btn response-btn" data-points="${item.points}">
                        <span style="width: 30px; height: 30px; border-radius: 50%; background: #e0e5ec; margin-right: 15px; display: flex; align-items: center; justify-content: center; font-size: 0.9rem; color: #636e72; box-shadow: inset 2px 2px 5px #d1d9e6, inset -2px -2px 5px #ffffff;">${String.fromCharCode(65 + index)}</span>
                        ${item.response}
                    </button>
                `).join('')}
            </div>
